...
6. **Weekend holidays are deducted twice.** `LeaveDatesScreen.fx.yaml` lines 250-253: `_requestedDays = _workDaysInRequest - _holidaysInRequest`, where `_holidaysInRequest` counts every non-working holiday in the range, weekend or not. A request covering only Saturday 15 Aug 2026 gives 0 - 1 = **-1** days.
